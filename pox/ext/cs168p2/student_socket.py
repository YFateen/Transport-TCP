...
class StudentUSocket(StudentUSocketBase):
  ...
  def connect(self, ip, port):
    ...
    synPacket = self.new_packet(False, None, True)
    synPacket.tcp.seq = self.snd.iss
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    indicatorBoolean = not self.rx_queue.empty()
    while indicatorBoolean:
      if (self.rx_queue.peek()[0] |GT| self.rcv.nxt):
        self.set_pending_ack()
        indicatorBoolean = False
      else:
        poppedPacket = self.rx_queue.pop()[1]
        self.handle_accepted_seg(poppedPacket.tcp, poppedPacket.app)
        indicatorBoolean = not self.rx_queue.empty()
  ...
  def update_rto(self, acked_pkt):
    ...
    if self.srtt == 0:
      self.srtt = self.stack.now - acked_pkt.tx_ts
      ...
      newRTT = self.stack.now - acked_pkt.tx_ts
  ...
  def handle_accepted_seg(self, seg, payload):
    ...
    # Complete for Stage 2
    if (self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2)) and (len(seg.payload) |GT| 0):
  ...
  def maybe_send(self):
    ...
    bytes_sent = 0
    # Complete for Stage 4
    # SHould be updayed tx_data
    windowSize = self.snd.wnd |MINUS| (self.snd.nxt |MINUS| self.snd.una)
    dataSize = min(windowSize, len(self.tx_data))
    # pdb.set_trace()
    while dataSize > 0:
      tempSize = min(self.mss, dataSize, self.tx_data)
      payload = self.tx_data[:tempSize]
      self.tx(self.new_packet(True, payload, False))
      dataSize -= tempSize
      self.tx_data = self.tx_data[tempSize:]
      num_pkts += 1
      bytes_sent += tempSize
      windowSize = self.snd.wnd |MINUS| (self.snd.nxt |MINUS| self.snd.una)
  ...
  def check_timer_retx(self):
    ...
    if not self.retx_queue.empty():
      earliestPacket = self.retx_queue.get_earliest_pkt()[1]
      time_in_queue = self.stack.now - earliestPacket.tx_ts
      if time_in_queue > self.rto:
        earliestPacket.tx_ts = max(earliestPacket.tx_ts * 2, self.MAX_RTO)
        self.log.debug("earliest packet seqno={0} rto={1} being rtxed".format(earliestPacket.tcp.seq, self.rto))
        self.tx(earliestPacket, retxed=True)
